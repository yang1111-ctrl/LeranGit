...
//上报视频、资讯播放次数
function uploadTotalPlay(id,Type,iFrom,sControl){
	if(iFrom == 1 && !sControl){
		var timer = setInterval(function(){
			if(LWPlayer.getPlaytime() > 0){
				DoTotalPlay(id,1,function(data){});
				clearInterval(timer);
			}
		},500);	
	}else{
...
function DoTotalPlay(id,Type,callback){
	var postdata = {
        'p0' : G_biz,
        'p1' : 'updateTotalPlay',
        'p2' : id,
...
function QueryTargetInfo(callback){		
    var rectaget = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_VDRECTYPE_GW_'+G_biz+'.js','rectargetTypeObj');
    rectaget.then(function(data){
    	var promisearr = [];
    	$.each(data.msg,function(i,v){
    		var p = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_VDRECLIST_GW_'+G_biz+'_'+v['iTarget']+'.js','rectargetObj_'+v['iTarget']);
    		promisearr.push(p);
    	});
...
function QueryNewsTargetInfo(callback){		
    var rectaget = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_NEWSRECTYPE_GW_'+G_biz+'.js','NewsrectargetTypeObj');
    rectaget.then(function(data){
    	var promisearr = [];
    	$.each(data.msg,function(i,v){
    		var p = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_NEWSRECLIST_GW_'+G_biz+'_'+v['iTarget']+'.js','NewsrectargetObj_'+v['iTarget']);
    		promisearr.push(p);
    	});
    	$.when.apply($, promisearr).then(function(){
			var retData = Array.prototype.slice.call(arguments);
...
function QueryUserTargetInfo(callback){		
    var rectaget = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_USERRECTYPE_GW_'+G_biz+'.js','rectargetTypeObj');
    rectaget.then(function(data){
    	var promisearr = [];
    	$.each(data.msg,function(i,v){
    		var p = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_USERRECLIST_GW_'+G_biz+'_'+v['iTarget']+'.js','rectargetObj_'+v['iTarget']);
    		promisearr.push(p);
    	});
    	$.when.apply($, promisearr).then(function(){
			var retData = Array.prototype.slice.call(arguments);
...
function QueryUserRankList(callback){
	var userrankObj = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_USERRANKLIST_GW_'+G_biz+'.js','userrankObj');
...
function QueryHotKeywords(callback){
	var hotkeywordObj = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_HOTKEYWORDLIST_GW_'+G_biz+'.js','hotkeywordObj');
...
function QueryKeywordList(callback){
	var keywordObj = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_KEYWORDLIST_GW_'+G_biz+'.js','keywordObj');
...
function QueryNewsHotKeywords(callback){
	var hotkeywordObj = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_NEWSHOTKEYWORDLIST_GW_'+G_biz+'.js','newshotkeywordObj');
...
function QueryNewsKeywordList(callback){
	var keywordObj = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_NEWSKEYWORDLIST_GW_'+G_biz+'.js','newskeywordObj');
...
function QueryParentTypeInfo(callback){
	var p = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_PARENTTYPE_GW_'+G_biz+'.js','ParentType');
	p.then(function(data){
		callback(data.msg);
	});
	return p;
...
function QueryNewsParentTypeInfo(callback){
	var p = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_NEWS_PARENTTYPE_GW_'+G_biz+'.js','NewsParentType');
	p.then(function(data){
		callback(data.msg);
	});
	return p;
...
function QueryTypeInfo(callback){
	var typeInfo = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_TYPELIST_GW_'+G_biz+'.js','typeObj');
...
function QueryNewsTypeInfo(callback){
	var typeInfo = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_NEWS_TYPELIST_GW_'+G_biz+'.js','newsTypeObj');
	typeInfo.then(function(typeObj){
		callback(typeObj.msg);
	});
	return typeInfo;
...
function QueryTypeList(callback){
	...
	typeInfo.then(function(data){
		...
		$.each(data.msg,function(i,v){
			if(i.split('_')[1] != 0){
				var p = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_VIDEOLIST_GW_'+G_biz+'_'+i.split('_')[1]+'.js','resObj_'+i.split('_')[1]);
    			promisearr.push(p);
			}   		
    	});
    	$.when.apply($, promisearr).then(function(){
...
function QueryNewsTypeList(callback){
	...
	typeInfo.then(function(data){
		...
		$.each(data.msg,function(i,v){
			if(i.split('_')[1] != 0){
	    		var p = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_NEWSLIST_GW_'+G_biz+'_'+i.split('_')[1]+'.js','newsResObj_'+i.split('_')[1]);
	    		promisearr.push(p);
    		}
    	});
    	$.when.apply($, promisearr).then(function(){
...
function QueryNewList(callback){
	var p = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_RANKLIST_GW_'+G_biz+'.js','rankObj');
	p.then(function(data){
...
function QueryNewsNewList(callback){
	var p = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_NEWS_RANKLIST_GW_'+G_biz+'.js','newsRankObj');
	p.then(function(data){
		callback(data.msg.newlist);
	});
}
...
function QuerySpecialList(callback){
	var p = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_SPECIALIST_GW_'+G_biz+'.js','specialObj');
	p.then(function(data){
		callback(data);
	});
}
...
function QueryWeeklyRankList(callback){
	var JFRankObj = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_JFRANKLIST_GW_'+G_biz+'.js','JFRankObj');
...
function QueryWeeklyRankList(callback){
	var JFRankObj = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/v3/WMP_NEWS_JFRANKLIST_GW_'+G_biz+'.js','NesJFRankObj');
...
function QueryDetailInfo(id,callback){
	var BizInfo = postScriptDeferStaticData('//ams.qq.com/wmp/data/js/PAGE_WMP_BIZ_INFO.js','BizInfo');
...
function CreateCommentIframe(ElementId,source){            
    ...
    iframe.src = '//www.qq.com/coral/coralBeta3/coralMainDom3.0.htm';      
...
function GetQVideoPlay(vid,ElementId,iFrom,url,bullet) {
...
function CreateVideoIframe(vid,ElementId,width,height,iFrom,url){   
    ...
    if(iFrom != 1){
    	...
    	iframe.src = '//ams.qq.com/wmp/v/wmplayer.html?vid='+vid+'&width='+width+'&height='+height;  
    	document.getElementById(ElementId).appendChild(iframe);  
    	var timer = setInterval(function(){
...
//创建H5播放标签
function CreateVideoPlay(ElementId,width,height,url){   
...
function WMP_LOG_EAS(p1,p2,p3,p4,p5){
	...
	var p4 = +p4;
	loadScript("//ossweb-img.qq.com/images/js/eas/eas.js",function(){
		pgvSendClick({hottag:''+p1+'.'+p2+'.'+p3+'.'+p4+''});
		EAS.VShow({
				'VUrl':p5,
		        'Vid': p4,
...
function reportToCfMobile(msg){
    var url = '//apps.game.qq.com/wmp/v3.1/index.php?p0=1&p1=reportToCfMobile';
    var key = msg.sVID;
    if (msg.sExt3 != ''){
        key = msg.sExt3;
    }
...
/*  |xGv00|bdcf38508ba49c6f4aee14ada954ee41 */